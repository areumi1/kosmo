...
// 클래스(붕어빵기계) 와 객체의(붕어빵) 차이점
//	클래스 : 객체를 만들기 위한 틀, 붕어방 기계, 설계도
	// 필드 : 클래스 블락에서 선언된 변수 (멤버변수)
	// 생성자
	// 메소드 : 메소드 내부에서 선언된 변수 (지역변수)
	// 이너클래스 (내부 클래스)
//	객체(인스턴스) : 클래스를 트로해서 만들어진  Ram에 로드된 객체

// 클래스를 객체로 만드는 작업을 인스턴스화

// 외부클래스
class A{
	//필드 : Heap 공간에 값이 지정됨, 값을 넣지 않으면 강제 초기화가 됨
	int m =3;
	int n = 4;
...
public class Using_Class02 {
	...
	public static void main(String[] args) {
		...
		//객체 생성은 main 메소드에서 작정
		
		// 객체 생성
		A a = new A();
		// A : 외부클래스 A를 뜻함 : 참조 변수 타입
		// a : 객체 (참조변수) 
