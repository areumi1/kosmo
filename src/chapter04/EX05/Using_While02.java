...
public class Using_While02 {
	...
	public static void main(String[] args) {
		...
		int sum = 0;		// 입력 받는 정수를 모두 더한값
		
		System.out.println("원하는 정수를 입력 하신 뒤 마지막에 -1로 종료를 알리세요. : ");
		int n = sc.nextInt();		// 콘솔에서 정수값을 입력 받아 n 변수에 할당.
		
		// 반복 횟수를 알 수 없는 경우 : while
		while( n != -1) {		// 입력받은 정수 값이 -1 이 아니면 while 문을 실행
			sum += n;			// sum = sum + n;
			count++;		// 정수 값을 count 함.
			n = sc.nextInt();		//  -1이 정수 값으로 input 이 되면 루프를 빠져나온다.
		}
		if (count==0) {		// 제일 처음에 정수 값을 -1 을 넣은경우.
			System.out.println("입력된 정수가 없습니다.");
		}else {		// 정수 값을 하나이상 넣은 경우
