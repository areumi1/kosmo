...
public class Using_String05 {
	...
	public static void main(String[] args) {
		// String 의 주요한 특징 : 다른 클레스에는 없는 특별한 기능
		//		1. 수정이 불가하다. ==> 새로운 값을 힙영역에 생성 
		//		2. 리터널로 변수의 값을 넣을 경우 동일한 문자열은 주소를 공유
		...
		System.out.println(str2.length());		//13자		<== 글자수  자바에선 한글도 1byte
		System.out.println("======================");
		
		//2. 문자열 검색 (charAt()), intdexOf(), lastIndexOf())
		// 2-1. chatAt() : 해당 index(방번호)의 문자를 가져옴 
		...
		//	2-2. indexOf() : 해당 글자에 대한 방번호를 출력, 왼족(처음) ==> 오른쪽(끝)
		//		 lastIndexOf() : 해당 글자에 대한 방번호를 출력, 오른쪽(끝) ==> 왼족(처음)
		...
		String str5 = str3.concat(str4);		// + 와 같음 문자열 연결
		...
		System.out.println(Arrays.toString(arr2)); 		// 문자 ==> 숫자 숫자코드로 변환	
		
		System.out.println("================");
		
		// 문자열을 char[] 배열에 저장		 많이씀!!
