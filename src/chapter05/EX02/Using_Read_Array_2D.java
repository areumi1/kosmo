...
public class Using_Read_Array_2D {
	...
	public static void main(String[] args) {
		...
		// 2. 방법2 : for문을 사용해서 출력 : 2차원 배열은 이중 for문 (for문 내의 for문) 
			// 바깥쪽 for 문 : arr.length : 행의 갯수
			// 안쪽 for 문 : arr[i].length : 열의 갯수
		...
		for (int i = 0; i < arr2.length; i++) {		// 행의 갯수를 1씩 증가
			...
			System.out.println();	// 한행이 끝나면 enter 적용
		}
		
		
		// 3. 방법3 : Enhanced for 문을 사용해서 출력 : 이중 for문을 사용해서 출력
				// 바깥쪽 for 문 : 
		
		System.out.println("== Enhanced for 문으로 출력 ==");
		for(int [] a : arr2) {		// arr2 이차원 배열을 일차원 배열로 꺼낸다.
