...
// 클래스 내부 구성요소 : 1. 필드, 2. 생성자, 3. 메소드, 4. 이너클래스(클래스 내부의 클래스)

// 클래스 외부 구성요소 : 1. 패키지, 2. import, 3. 외부클래스

// 상속(Inheritance) : 객체지향 언어의 꽃 (제일 중요)
...
// 자식 클래스 : 부모 클래스의 필드와 메소드를 그대로 상속 받음. 
		//	- 자식 클래스 만의 특징이 적용된 필드와 메소드르 선언 한다.

class Student extends Human {			// Student class는 Human class를 상속 받는다.
	
	// 부모클래스의 필드와 메소드가 그대로 내려온다.
	// 자식 클래스의 특징을 가진 필드와 메소드를 선언
		int studentID;			// 자식 클래스에서만 사용가능한 필드
