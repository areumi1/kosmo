...
public class Using_Polymorphism02 {
	...
	public static void main(String[] args) {
		...
		//3. Tiger 객체를 Animal 타입으로 생성 // 업캐스팅
			// a2은 Animal, Tiger를 내포 하고 있고, Animal 타입으로 지정
			// a2은 Animal 클래스의 필드와 메소드만 접근 가능 
		Animal a2 = new Animal();
		
		// 4. Dog 객체를 Dog 타입으로 생성 
		// d1은 Animal, Dog를 내포 하고 있고, Dog 타입으로 지정
		// d1은 Animal, Dog 의 필드와 메소드 모두 접근
		Dog d1 = new Dog();
		
		// 5. Dog 객체를 Animal 타입으로 생성
		// a3은 Animal, Dog 를 내포 하고 있고, Animal 타입으로 지정
		// a3은 Animal 클래스의 필드와 메소드만 접근 가능 
		Animal a3 = new Dog();
		
		// 6. Dog_sub 객체를 Dog_sub 타입으로 생성 
		// ds1은 Animal, Dog, Dog_sub 를 내포 하고 있고, Dog_sub 타입으로 지정
		// ds1은 Animal, Dog, Dog_sub 의 필드와 메소드 모두 접근
		Dog_sub ds1 = new Dog_sub();
		
		// 7. Dog_sub 객체를 Dog 타입으로 생성
		// a3은 Animal, Dog, Dog_sub 를 내포 하고 있고, Dog 타입으로 지정
		// ds1은 Animal, Dog 의 필드와 메소드 모두 접근 
		Dog d2 = new Dog_sub();
		
		// 8. Dog_sub 객체를 Animal 타입으로 생성
		// a4은 Animal, Dog, Dog_sub 를 내포 하고 있고, Animal 타입으로 지정
		// a4은 Animal 클래스의 필드와 메소드만 접근 가능 
