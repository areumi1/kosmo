...
// class 외부에 올수 있는것 : 패키지, 임포트( 같은 패키지에 존재하지 않는 클래스), 외부클래스

class A{	//클래스 : 객체를 생성하기 위한 설계도
	//클래스 내부에 올수 있는 것(4가지) 
	//1. 필드 (클래스에서 선언된 변수) 2. 생성자 3. 메소드 4. 인너클래스
	//1. 필드
	int m ;		//0     //Heap 공간에 저장, 기본값 미입력시 강제초기화 
	int n ;
	double k;	//0.0
	String l;	//null
	
	//2. 생성자 : 클래스는 반드시 생성자가 존재해야 한다. 	특징 : 1. 클래스 이름과 동일 2. 리턴이 존재하지 않아야 된다.
	// 			기본생성자(입력 매개변수가 없는 생성자)는 생략 될 수 있다. 	A(){}	() 안에 매개변수 입력
	//			객체 생성시 생성자 호출
	//			객체의 초기값을 셋틍
