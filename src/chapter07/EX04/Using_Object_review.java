...
class Person{
	...
	// 1. 필드 : class 블락에서 선언된 변수(heap 에 저장). 	지역변수 : 메소드 블락에서 선언된 변수. (stack에 저장)
				// 필드는 힙에 저장되므로 기본값 자동 초기화
	String name;	// 이름				: null
	...
	// 2. 생성자 (Constructor) : 1. 리턴타입이 없다. 	2. 클래스 이름과 동일 (메소드)	
				// 필드의 기본값을 초기화 시켜주는 역활
				// 생성자 오버로딩 : 생성자 이름은 동일, 식별자(시그니쳐) (인풋매개변수 타입, 갯수)
	Person () {			// 기본생성사 : 인풋 값이 비어있는 생성자 , 생략가능	
	}
	Person (String s){		// this 키워드 없이 필드에 값 할당. 변수명이 다르다.
		name = s;
	}
	Person (String name, float height){		// this : 인풋 매개변수명, 값을 받는 변수, 필드명 같을때
		this.name = name;
		this.height = height;
	}
	...
	// 4. getter (필드의 내용을 출력), setter	(필드의 내용을 입력)			<== 필드의 내용을 출력, 필드의 내용을 입력
...
public class Using_Object_review {
	...
	public static void main(String[] args) {
		...
		System.out.println(person.age);
			...
			// 필드 내용 출력 (2. getter 를 사용해서 필드의 내용을 출력 : 필드의 값을 리턴)
		...
		// 2. 매개변수가 1개인 생성자 호출
		System.out.println("=== 매개변수가 1개인 생성자 출력 ===");
		...
		System.out.println("=== 매개변수가 4개인 생성자 출력 ===");
		...
		//setter 를 사용해서 필드의 값 할당.
		jung.setName("정도령");
		jung.setHeight(175.5F);
		jung.setWeight(85.3F);
		jung.setAge(40);
		
		//getter 를 사용해서 필드의 값 호출.
