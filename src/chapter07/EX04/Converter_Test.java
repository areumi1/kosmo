...
class Converter{		//환율을 계산하는 클래스
	...
	//3. 메소드 생성 (원화를 인풋받아서 달러를 반환하는 메소드), 메소드 이름 : toDoller()
	double toDoller(double won){
		return won / rate;
	}
	//4. setRate() 메소드 생성 : setter를 사용해서 rate 필드의 값을할당.
...
public class Converter_Test {
	...
	public static void main(String[] args) {
		...
		System.out.println("1$에 대한 환률을 입력해 주세요 >>>");
		...
		// 100만원 ==> 몇 $
		System.out.println("원화 100만원은 " + converter.toDoller(1000000) + " $입니다.");
		...
		System.out.printf("원화 100만원은 %.2f $입니다. \n",converter.toDoller(1000000));
		System.out.printf("100$는 %.2f 원 입니다.",converter.toKWR(100));
		...
		System.out.println("==실수를 변수에 할당후 소숫점 2자리까지 자른후 다시 더블에 저장==");
		double d2 = converter.toDoller(1000000);		// 실수를 리턴 받아서 d2에 할당.
