...
public class LocalOperator {
	...
	public static void main(String[] args) {
		...
		//XOR 연산 : ^		<== 좌우의 값이 동일할때 false, 좌우 값이 다를때 true
		...
		int value1 = 3;
		System.out.println(false && ++value1 > 6);	// short circuit		// 앞이 false이므로 뒤에것을 실행하지않음
		System.out.println(value1);		//3
		
	
		System.out.println(true && ++value1 > 6);		// 앞이 true이기 때문에 뒤에것이 실행 
		System.out.println(value1);		//4
		
		// 비트연산은 short circuit이 발생안됨
		int value2 = 3;
		System.out.println(false & ++value2 > 6); 		// 비트연산 이므로 short circuit 실행안되고 앞, 뒤 모두 확인 
